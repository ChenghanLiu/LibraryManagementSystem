        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('tushujieyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tushuguanli/upload/tushujieyueMuBan.xls"
                        >批量导入借阅信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tushujieyue','导入导出')"
                                ...
                                :on-success="tushujieyueUploadSuccess"
                                :on-error="tushujieyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('tushujieyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入借阅信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('tushujieyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "tushujieyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushujieyueShijian"
                                      header-align="center"
                                      label="借阅时间（天）">
                        <template slot-scope="scope">
                            {{scope.row.tushujieyueShijian}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushujieyueTypes"
                                      header-align="center"
                                      label="借阅状态">
                        <template slot-scope="scope">
                            {{scope.row.tushujieyueValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('tushujieyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('tushujieyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('tushujieyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('tushujieyue','还书') && scope.row.tushujieyueTypes == 1" type="primary" icon="el-icon-edit" size="mini" @click="huanshu(scope.row.id)">还书</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                tushuId : null,
                yonghuId : null,
                tushujieyueShijian : null,
                tushujieyueTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '邮箱': 'yonghuEmail',
                //本表字段
                     '借阅时间（天）': "tushujieyueShijian",
                     '借阅状态': "tushujieyueTypes",
                     '借阅时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"tushujieyue",//当前表表名,
                        sumColum : 'tushujieyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'tushujieyue_name',//分组字符串字段
                        // types : 'tushujieyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['tushujieyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            tushujieyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "tushujieyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
